...
public class Shop {
    ...
    public static void getMenu() {
        ...
        if(sc.hasNextInt()) {
            ...
            switch (number) {
                case 1 -> getCategory(all);
                case 2 -> getBasket(card);
                case 3 -> Buy();
                case 4 -> outString();
                case 0 -> {}
    ...
    public static void Buy() {
        ...
        for(int i = 0; i < client.purchased.purchased.length; ++i) {
            ...
            result.append(i + 1).append(".").append(client.purchased.purchased[i].toString()).append("\n");
        }
        double gc = getCash(client.purchased.purchased);
        System.out.print("Общая сумма покупок составляет = " + gc + " руб.");
        result.append("Общая сумма покупок составляет = ").append(gc).append(" руб.");

        try {
            FileOutputStream outputStream = new FileOutputStream("lesson7_8\\Basket.out");
    ...
    public static void outString(){
        try {
            FileInputStream fileInputStream = new FileInputStream("lesson7_8\\Basket.out");
            try {
    ...
    public static double getCash(Product[] p) {
        ...
        for (Product product : p) {
            ...
            cash += product.cost * product.count;
        }
        return cash;
    }
