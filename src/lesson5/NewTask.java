...
public class NewTask {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Урок 5: работа со строками.");
        int num = 1;
        while (num != 0) {
            System.out.print("Введите номер задачи (введите <0> или букву для выхода): ");
            if (!in.hasNextInt()) {
                System.out.print("Выход из программы. ");
                num = 0;
            }
            else num = in.nextInt();
            //Дополнительный метод nextLine() нужен, чтобы объект типа Scanner
            //не считывал символ Enter-а после метода nextInt()
            in.nextLine();
